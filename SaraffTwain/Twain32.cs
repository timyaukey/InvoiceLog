    ...
    public sealed class Twain32:Component {
        ...
        public bool SelectSource()
        {
            if ((this._TwainState & TwainStateFlag.DSOpen) == 0)
            {
                ...
                TwRC _rc = this._dsmEntry.DsmInvoke(this._AppId, TwDG.Control, TwDAT.Identity, TwMSG.UserSelect, ref _src);
                //TJY - added the condition for TwRC.Cancel.
                if (_rc == TwRC.Cancel)
                    return false;
                if (_rc == TwRC.Success)
                {
        ...
        {
            get { return this._srcds.ProductName; }
        }
        ...
        public bool CloseDataSource()
        {
            if ((this._TwainState & TwainStateFlag.DSOpen) != 0 && (this._TwainState & TwainStateFlag.DSEnabled) == 0)
            {
                TwRC _rc = this._dsmEntry.DsmInvoke(this._AppId, TwDG.Control, TwDAT.Identity, TwMSG.CloseDS, ref this._srcds);
                if (_rc != TwRC.Success)
                {
                    //TJY - Don't throw exception if status is "Success".
                    //TJY - Found empirically that if "FreeImage" data source fails the "Enable" operation
                    //TJY - then "CloseDS" will return "Failure" yet the status will be "Success".
                    TwCC status = this._GetTwainStatus();
                    if (status!= TwCC.Success)
                        throw new TwainException(status, _rc);
                }
                this._TwainState &= ~TwainStateFlag.DSOpen;
                return (this._TwainState & TwainStateFlag.DSOpen) == 0;
            }
